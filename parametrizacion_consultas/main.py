...
from utils.db_operations import create_and_format_personalized_dataframes, create_universal_dataframe

def main():
    print("Hello, world!")

    query_result_dict = create_and_format_personalized_dataframes(query_templates, asesores_info)
    send_dataframe_as_email(query_result_dict, SMTP_USER, SMTP_PASSWORD, SMTP_SERVER, SMTP_PORT)
    query_result_dict = create_universal_dataframe(query_universal, lista_correos)
    send_dataframe_as_email(query_result_dict, SMTP_USER, SMTP_PASSWORD, SMTP_SERVER, SMTP_PORT)
